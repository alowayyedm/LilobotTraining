...
<template>
  <div class="wrapper">
    ...
    <div class="second"><belief-input ref="beliefInput" :beliefs="beliefs" :previousValues="previousValues"
        :sessionActive="sessionActive" :phase="phase" :lastTransition="lastTransition" @update-belief="updateBelief"
        :privateSession="privateSession" @switch-to-phase="switchToPhase">
      </belief-input></div>
    <div class="third">
      <belief-transitions-component ref="beliefTransitions">
      </belief-transitions-component>
    </div>
    <div class="fourth"><web-chat-component ref="webChat"
        :header-text="`${sessionActive ? `${sessionActive}'s Chat` : 'You are not watching anyone'}`"
        @handle-input="handleInput" @set-trainer-messaging="setTrainerMessaging" @request-session="requestSession"
        @leave-session="leaveSession" @start-private-session="startPrivateSession" :isTrainingPortal="true"
        :scenario="scenario" @change-scenario="changeScenario" @get-scenario="getScenario">
      </web-chat-component>
    </div>
  </div>
  <join-popup ref="joinPopup" @send-join-request="sendJoinRequest">
  </join-popup>
  <generic-popup ref="genericPopup" accept-text="ACCEPT" decline-text="DECLINE">
  </generic-popup>
</template>
...
export default {
  ...
  methods: {
    ...
    changeScenario(scenario) {
      ...
      this.fetchInitBeliefs();
      editorApi.setUserScenario(this.getSessionID(), scenario);
    },
    getScenario() {
      editorApi.getUserScenario(this.getSessionID()).then(scenario => {
        this.scenario = scenario;
      });
    },
    ...
    fetchInitBeliefs() {
      axios.get(`${this.$config.agentServer}/beliefs/all/scenario/${this.scenario}`, {
        headers: {
          'Authorization': 'Bearer ' + this.$store.state.auth.token
        }
      })
    ...
    processReceivedUpdate(message) {
      ...
      if (updateType === "UNCHANGED") {
        if (logAhead) {
          this.$refs.beliefTransitions.addMsgTransition(logIndex, msgText, causeType, {
            positiveBeliefs: [],
            negativeBeliefs: []
          });
        }
